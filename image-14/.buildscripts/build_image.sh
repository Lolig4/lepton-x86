...
pushd "${ROOT_DIR}/output" >/dev/null
    ...
    # Set target
    lunch "${1:-aosp_lepton_arm64_only-ap2a-userdebug}"
    get_build_var BUILD_FINGERPRINT

    num_procs=$(nproc)
    if [[ "$IS_CI" == "true" ]]; then
        make installclean -j${num_procs} | grep --line-buffered -o "[0-9]\+%" | stdbuf -oL -eL uniq || true
        echo "Starting build... (logs will be in output/out/verbose.log.gz)"
        make systemimage vendorimage -j${num_procs} 2>&1 | grep --line-buffered -o "[0-9]\+%" | stdbuf -oL -eL uniq
    else
        make installclean -j${num_procs} || true
        make systemimage vendorimage -j${num_procs} 2>&1
    fi
    ./prebuilts/build-tools/path/linux-x86/python build/tools/generate-notice-files.py -s "${OUT}/obj/NOTICE_FILES/src" --title "Lepton Android Licenses" --text "${OUT}/NOTICE.txt"
