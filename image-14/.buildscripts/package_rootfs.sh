...
# Convert apex to flattened apex
function flatten_apexes()
{
    shopt -s nullglob
    ...
    for f in *.apex; do
        fnosuffix="${f%.apex}"
        mkdir -p "$f.unzip"
        mkdir -p "${fnosuffix}" "$f.mount"
        unzip $f -d "$f.unzip" >/dev/null
        if [[ -n "${GITLAB_CI:-}" ]]; then
            debugfs -R "rdump / $f.mount" "$f.unzip/apex_payload.img"
        else
            fuse2fs -o fakeroot "$f.unzip/apex_payload.img" "$f.mount"
        fi
        rsync -a --exclude='lost+found' "$f.mount"/* "${fnosuffix}"/
        chown -R $(id -u):$(id -g) "${fnosuffix}"
        if [[ -z "${GITLAB_CI:-}" ]]; then
            fusermount3 -u "$f.mount"
        fi
        rm -r "$f.unzip"
        rm -r "$f.mount"
        rm "$f"
...
pushd "${OUTPUT_DIR}"/rootfs/system/apex >/dev/null
flatten_apexes
popd >/dev/null

pushd "${OUTPUT_DIR}"/rootfs/vendor/apex >/dev/null
flatten_apexes
popd >/dev/null

find . -type f -iname '*Bluetooth*.apk' -delete
